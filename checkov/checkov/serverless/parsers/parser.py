...
def process_intrinsic(template):
    def process_intrinsic_helper(obj_iterator, obj, original):
        for key, value in obj_iterator():
            if isinstance(value, dict):
                process_intrinsic_helper(value.items, value, original)
                for inner_key in value: # the inner dict is supposed to be length 1 but it's actually 3 because of aux keys
                    if is_intrinsic_fn(inner_key):
                        resolved_value = resolve_intrinsic_fn(inner_key, value[inner_key], original)
                        obj[key] = resolved_value
            elif isinstance(value, list):
                process_intrinsic_helper(lambda: enumerate(value), value, original)
    process_intrinsic_helper(template.items, template, template)


def resolve_intrinsic_fn(key, value, full_template):
    if key == 'Fn::Join':
        resolved_value = value[0].join(value[1])
        return resolved_value
    if key == 'Ref':
        # Just return the value since I can't get access to anything else anyway
        # this should be consistent with how I use name resources in the Prolog output
        return value
    if key == 'Fn::Base64':
    ...
    if key == 'Fn::GetAZs':
        # Hard coded garbage. don't think it matters
        return [ "us-east-1a", "us-east-1b", "us-east-1c", "us-east-1d" ]

    if key == 'Fn::Select':
        idx = int(value[0])
        if isinstance(value[1], list) and len(value[1]) > idx:
            return value[1][idx]
        else:
            return ''
    ...
    raise NotImplementedError('Intrinsic Function {} not implemented'.format(key))


def is_intrinsic_fn(value):
    # Because some keys can be int e.g., http status codes 404...
    if not isinstance(value, str) and not isinstance(value, StrNode):
        return False

    if value.startswith('Fn::') or value == 'Ref':
        return True
    return False
